...
import { getMyTrades, TradeItem } from '../../../services/api/modules/trades';
...
const MyTrade: React.FC<MyPlaceProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    const fetchTrades = async () => {
      try {
        ...
        console.error('내 거래 내역 조회 실패:', err);
        setError('내 거래 내역을 불러오는 데 실패했습니다. 나중에 다시 시도해주세요.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <Container>
      <Header title="내 거래 내역" showBackButton={true} onBack={handleBack} />
      <HeaderBackground />

      <Content>
        {loading ? (
          <LoadingContainer>내 거래 내역을 불러오는 중...</LoadingContainer>
        ) : error ? (
          <ErrorMessage>{error}</ErrorMessage>
        ) : trades.length === 0 ? (
          <NoDataMessage>등록된 거래 내역이 없습니다.</NoDataMessage>
        ) : (
          trades.map(trade => (
            <TradeItemCard
              key={trade.trade_id}
